...
BEGIN
    ...
    IF table_exists THEN
        ...
        IF NOT id_exists THEN
            RAISE NOTICE 'Добавляем столбец id в таблицу advices...';
            ALTER TABLE public.advices ADD COLUMN id UUID PRIMARY KEY DEFAULT uuid_generate_v4();
        END IF;
...
-- Проверка структуры таблицы users и добавление недостающих столбцов
DO $$
DECLARE
    table_exists BOOLEAN;
    id_exists BOOLEAN;
...
BEGIN
    ...
    IF table_exists THEN
        ...
        -- Добавляем недостающие столбцы
        IF NOT id_exists THEN
            RAISE NOTICE 'Добавляем столбец id в таблицу users...';
            ALTER TABLE public.users ADD COLUMN id UUID PRIMARY KEY DEFAULT uuid_generate_v4();
        END IF;

        IF NOT role_exists THEN
            RAISE NOTICE 'Добавляем столбец role в таблицу users...';
...
-- Создание новых политик с учетом структуры таблиц
DO $$
DECLARE
    users_id_exists BOOLEAN;
    users_role_exists BOOLEAN;
    advices_author_id_exists BOOLEAN;
BEGIN
    -- Проверяем наличие столбцов для политик
    SELECT EXISTS (
        SELECT FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'users' 
        AND column_name = 'id'
    ) INTO users_id_exists;
    ...
    SELECT EXISTS (
        ...
        AND table_name = 'advices' 
        AND column_name = 'author_id'
    ) INTO advices_author_id_exists;

    -- Создаем базовые политики для всех таблиц
    ...
    EXECUTE 'CREATE POLICY advices_select_policy ON public.advices FOR SELECT USING (true)';
    EXECUTE 'CREATE POLICY advices_insert_policy ON public.advices FOR INSERT WITH CHECK (true)';

    -- Создаем политики обновления и удаления с учетом структуры таблиц
    IF users_id_exists AND users_role_exists THEN
        -- Полные политики для users
        EXECUTE '
        ...
        ';
    ELSE
        -- Упрощенные политики для users
        EXECUTE 'CREATE POLICY users_update_policy ON public.users FOR UPDATE USING (true)';
        EXECUTE 'CREATE POLICY users_delete_policy ON public.users FOR DELETE USING (true)';
    END IF;

    IF advices_author_id_exists AND users_id_exists AND users_role_exists THEN
        -- Полные политики для advices
        EXECUTE '
        ...
        ';
    ELSE
        -- Упрощенные политики для advices
        EXECUTE 'CREATE POLICY advices_update_policy ON public.advices FOR UPDATE USING (true)';
        EXECUTE 'CREATE POLICY advices_delete_policy ON public.advices FOR DELETE USING (true)';
    END IF;
END $$;
